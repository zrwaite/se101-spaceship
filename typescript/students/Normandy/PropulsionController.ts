...
export default class YourPropulsionController extends PropulsionController {
	...
	propulsionUpdate(setThruster: (thruster: ThrusterName, power: number) => Error | null) {
        ...
        if (Math.abs(headingDiff) < 0.05 || this.didsetThruster && Math.abs(this.navigation.angularVelocity)<0.0001) {
            setThruster('clockwise', 0)
            setThruster('counterClockwise', 0);
            this.didsetThruster = false
        }
        else if (!this.didsetThruster) {
    ...
    calculateAccelAngle(currentAngVelo: number, currentAngle: number, targetAngle: number) {
        ...
        var val = 100000*Math.abs(currentAngVelo*currentAngVelo)/(2*angleDiff(currentAngle,targetAngle))
