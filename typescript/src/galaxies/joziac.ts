...
export const BuildJoziac = (game: Game): Galaxy => {
	...
	*/
	...
	const gatesAsteroids: Asteroid[] = []
	for (let x=0; x<15; x++) {
		for (let y=0; y<10; y++) {
			gatesAsteroids.push(
				new Asteroid(V2(0,0), V2(40 + 720 * x / 15, 20 + 540 * y / 10), game)
			)
		}
	}
	...
	const startingTorvaldsAsteroids:Asteroid[] = []
	for (let y=20; y<440; y+= 80) startingTorvaldsAsteroids.push(
		new Asteroid(V2(0, 1), V2(20, y), game),
		new Asteroid(V2(0, -1), V2(700, y+100), game)
	)
	for (let x=20; x<620; x+=80) startingTorvaldsAsteroids.push(
		new Asteroid(V2(-1, 0), V2(x+100, 20), game),
		new Asteroid(V2(1, 0), V2(x, 520), game)
	)
