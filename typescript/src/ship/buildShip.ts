...
const buildAllShips = (pos: Vector2, game: Game, startProcess: Process): ColonyShip[] => {
	const shipNames = [
		'Bebop',
		'Bismark',
		'Enterprise',
	...
	let ships: ColonyShip[] = []
	shipNames.forEach((shipName) => {
		let newShip = buildShip(shipName, pos, game, startProcess)
		if (newShip) ships.push(newShip) //Build each ship into array to return
		else throw Error('Failed to build ship')
	})
	return ships
}
const buildShip = (shipName: string, pos: Vector2, game: Game, startProcess: Process): ColonyShip => {
	switch (shipName) {
		...
		case 'Galactica':
			return new ColonyShip('Galactica', startProcess, GalacticaDefenceController, GalacticaNavigationController, GalacticaPropulsionController, GalacticaSensorsController, pos, game)
		case 'Milano':
		...
		default:
			throw Error('Invalid ship name: ' + shipName)
	}
}
