...
export default class TurretControls extends RenderedObject {
  ...
  getTubeCooldown(tubeIndex: number): number | Error {
    ...
    if (tubeIndex >= 0 && tubeIndex < NUMBER_OF_TUBES) {
      ...
      // Invalid tubeIndex
      return new Error(
        "getTubeCooldownFailed due to invalid tube index; expected tubeIndex from 0 (inclusive) up to " +
          NUMBER_OF_TUBES +
  ...
  fireTorpedo(tubeIndex: number): Error | null {
    ...
    if (tubeIndex >= 0 && tubeIndex < NUMBER_OF_TUBES) {
      ...
      if (tubeCooldownResponse instanceof Error) return tubeCooldownResponse;
        ...
        const relativeVelocity = new Vector2(0, this.launchSpeed).rotateTo(
          this.angle
        );
        const torpedoVelocity = relativeVelocity.add(this.parentShip.speed);
        const newTorpedo = new Torpedo(
          torpedoVelocity,
          this.parentShip.pos,
          this.parentShip.game
        );
        ...
        return new Error(
          "fireTorpedo failed due to internal call to getTubeCooldown not returning zero tubeCooldown response for tubeIndex " +
            tubeIndex
        );
      }
    } else {
      return new Error(
        "fireTorpedo due to invalid tube index; expected tubeIndex from 0 (inclusive) up to " +
          NUMBER_OF_TUBES +
          " (exclusive) but received " +
          tubeIndex
      );
