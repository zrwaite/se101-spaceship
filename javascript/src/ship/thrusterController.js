...
export default class ThrusterController extends RenderedObject {
    constructor(parentShip, game) {
        ...
        this.ctx = 'ships';
        this.manualControlDisabled = false;
        this.thrusterPower = {
            main: 0,
            bow: 0,
            clockwise: 0,
    ...
    draw() {
        ...
        for (const key in this.flames) {
            ...
            if (power) {
                ...
                ctx.drawImage(this.image, ((-(flame.size.x / 10) * this.game.unit) / 2) * this.game.zoom, ((-(flame.size.y / 10) * this.game.unit) / 2) * this.game.zoom, (flame.size.x / 10) * this.game.unit * this.game.zoom, (flame.size.y / 10) * this.game.unit * this.game.zoom);
                ctx.globalAlpha = 1;
            }
        }
    }
